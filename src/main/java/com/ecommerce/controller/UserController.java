...
public class UserController extends AbstractController<UserDto> {
    ...
    public ResponseEntity<UserDto> login(@RequestBody UserDto userDto) {
        UserDto loggedUser = authHandler.login(userDto.username(), userDto.senha());
        if (loggedUser != null) {
    ...
    public ResponseEntity<AddressDto> createAddress(@PathVariable Long id, @RequestBody AddressDto address) {
        ...
        // TODO: Implementar validação de id do usuário
    ...
    public ResponseEntity<UserDto> get(@PathVariable Long id) {
        // Implementação básica
        return ResponseEntity.ok(new UserDto(id, "user" + id, "User " + id, "user" + id + "@email.com", "password123", "11999999999", "Cliente", null, null, null));
    }
