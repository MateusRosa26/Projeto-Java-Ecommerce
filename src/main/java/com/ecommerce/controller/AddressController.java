...
public class AddressController extends AbstractController<AddressDto> {
    ...
    public ResponseEntity<AddressDto> createAddress(@PathVariable Long userId, @RequestBody AddressDto address) {
        ...
        if (!AddressValidator.isValidAddress(address.rua(), address.numero(), address.bairro(), 
                                            address.cidade(), address.estado(), address.cep())) {
            System.out.println("❌ Endereço inválido: " + AddressValidator.getValidationMessage());
            return ResponseEntity.badRequest().build();
        }
        
        System.out.println("✅ Endereço criado para usuário " + userId + ": " + address.rua() + ", " + address.numero());
    ...
    public ResponseEntity<List<AddressDto>> getAddressesByUser(@PathVariable Long userId) {
        ...
        // TODO: Implementar busca real por userId
