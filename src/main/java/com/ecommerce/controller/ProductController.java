...
public class ProductController extends AbstractController<ProductDto> {
    ...
    public ResponseEntity<ProductDto> findByBarcode(@PathVariable String barcode) {
        ProductDto product = businessHandler.findByBarcode(barcode);
        if (product != null) {
            return ResponseEntity.ok(product);
        }
        return ResponseEntity.notFound().build();
    ...
    public ResponseEntity<List<ProductDto>> searchByBarcode(@PathVariable String barcode) {
        List<ProductDto> products = businessHandler.searchByBarcode(barcode);
        return ResponseEntity.ok(products);
    }
}
